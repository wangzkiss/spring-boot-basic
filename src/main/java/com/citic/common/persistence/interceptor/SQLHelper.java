...
public class SQLHelper
{
    ...
    public static int getCount(final String sql, final Connection connection,
            ...
            final BoundSql boundSql, Log log, int countFlag) throws SQLException
    {
        
        final String countSql = 1 == countFlag
                ? "select count(1) from (" + sql + ") tmp_count"
                : "select count(1) " + removeSelect(removeOrders(sql));
        ...
        try
        {
            ...
            {
                log.debug("COUNT SQL: " + StringUtils.replaceEach(countSql,
                        new String[] { "\n", "\t" },
                        new String[] { " ", " " }));
            }
            if (conn == null)
            {
                conn = mappedStatement.getConfiguration()
                        .getEnvironment()
                        .getDataSource()
                        .getConnection();
            }
            ...
            {
                MetaObject mo = (MetaObject) Reflections.getFieldValue(boundSql,
                        "metaParameters");
                Reflections.setFieldValue(countBS, "metaParameters", mo);
            }
            //解决MyBatis 分页foreach 参数失效 end 
            SQLHelper.setParameters(ps,
                    mappedStatement,
                    countBS,
                    parameterObject);
            rs = ps.executeQuery();
